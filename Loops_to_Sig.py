...
class SignatureCreator( object):
    ...
    def __init__( self):
        ...
        ##### Added
        self.saveFilePath = GetInputFileMD5().lower() + "_loops.yar"
    ...
    def GetYaraSignature( self):
        ...
        if not self.__YaraSignature:
            self.__YaraSignature = []#['\nYARA SIGNATURE:\n']
            self.__YaraSignature.append( "rule loop_0x%x_0x%x\n" % (loopstart, loopend))
            self.__YaraSignature.append( "{\n\tmeta:\n")
            self.__YaraSignature.append( "\t\tsample_md5 = \"%s\"\n" % (GetInputFileMD5().lower()))
            self.__YaraSignature.append( "\t\tsubroutine = \"%s\"\n" % (GetFunctionName(funcea)))
            self.__YaraSignature.append( "\t\toffsets    = \"0x%x-0x%x\"\n\t\t\n" % (loopstart, loopend))
            #self.__YaraSignature.append( "\n")
            for head in Heads(self.__startAddress, self.__endAddress):
                self.__YaraSignature.append( "\t\t// %x: %s\n" % (head, GetDisasm(head)))
            self.__YaraSignature.append( "\t")
            self.__YaraSignature.append( "\n\tstrings:\n\t\t$hex_string = { ")
    ...
    def Run( self, loopstart, loopend):
        self.__startAddress = loopstart
        self.__endAddress = loopend
...
if __name__ == "__main__":
    ...
    for funcea in Functions(SegStart(ea), SegEnd(ea)):
        if GetFunctionName(funcea) not in imp:
            ...
            if loops:
                for loopend, loopstart in loops:
                    script = SignatureCreator()
                    script.Run(loopstart, ItemEnd(loopend))
